...
TARGET_PROGRAM_ADDRESS = "pAMMBay6oceH9fJKBRHGP5D4bD4sWpmSwMn52FMfXEA"
...
def transaction_matches_target(tx) -> bool:
    """Return True if any trade in tx targets the program address (bytes compare)."""
    for trade in tx.Trades:
        if trade.HasField("Dex"):
            if trade.Dex.ProgramAddress == TARGET_PROGRAM_BYTES:
                return True
    return False
        ...
        try:
            ...
            for tx in tx_block.Transactions:
                if transaction_matches_target(tx):
                    ...
                    logging.info("Match! Signature=%s", sig_b58)
                    # Uncomment to dump the whole tx:
                    # print_protobuf_message(tx, encoding="base58")
