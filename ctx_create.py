"""
Solana token creation filter (optimized)
...
def transaction_matches_target(tx) -> bool:
    """Return True if any instruction in tx targets the program address with target methods (bytes compare)."""
    for instruction in tx.ParsedIdlInstructions:
        if instruction.HasField("Program"):
            program = instruction.Program
            if program.Address == TARGET_PROGRAM_BYTES and program.Method in TARGET_METHODS:
                return True
    return False
    ...
    for message in batch_msgs:
        ...
        try:
            ...
            if tx_block.HasField("Header"):
                ...
                logging.info("New Block Message Received")
                logging.info("Block Slot: %s", header.Slot)
                logging.info("Block Hash: %s", base58.b58encode(header.Hash).decode())
                logging.info("Timestamp: %s", header.Timestamp)

            # Iterate transactions; log any matches
            for tx in tx_block.Transactions:
                if transaction_matches_target(tx):
                    sig_b58 = base58.b58encode(tx.Signature).decode()
                    logging.info("Matching Transaction Details:")
