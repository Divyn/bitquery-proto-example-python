"""
Pump.fun migrate detection (Solana)
...
PUMPFUN_MIGRATE_PROGRAM_BYTES = base58.b58decode(PUMPFUN_MIGRATE_PROGRAM_ID)

# Kafka client config: manual assignment => no group.id needed
BASE_CONF = {
...
def transaction_matches_target(tx) -> bool:
    """Return True if any instruction in tx is from Pump.fun migrate program with migrate log."""
    for instruction in tx.ParsedIdlInstructions:
        if instruction.HasField("Program"):
            # Check if this instruction is from Pump.fun migrate program (bytes compare)
            if instruction.Program.Address == PUMPFUN_MIGRATE_PROGRAM_BYTES:
                # Check logs for migrate instruction
                if instruction.Logs:
        ...
        try:
            ...
            for tx in tx_block.Transactions:
                ...
                if transaction_matches_target(tx):
                    ...
                    logging.info("Pump.fun migrate detected!")
                    logging.info("Transaction Signature: %s", sig_b58)
                    logging.info("Transaction Index: %s", tx.Index)
                    # Further logic based on your usecase

        except DecodeError as err:
            decode_errors += 1
            if decode_errors % 100 == 0:
                logging.warning("Total decode errors so far: %d", decode_errors)
